...
# Dictionaries für die Erkennung von Monatsnamen in Datumsangaben
MONTH_MAP_DE = {
...
def get_unique_item_identifier(item_data):
    """Erstellt einen eindeutigen Identifier für ein Paper, um Duplikate zu erkennen.
    Verwendet primär den Titel, normalisiert ihn aber stark.
    """
    title = item_data.get('title', '').strip().lower()
    if not title:
        return None # Kann nicht identifiziert werden, wenn kein Titel da ist
    # Entferne alle nicht-alphanumerischen Zeichen, um kleine Unterschiede zu ignorieren
...
def parse_date(date_str):
    ...
    try: return datetime.strptime(date_str_val, '%Y-%m-%d')
    except ValueError: pass
    try: return datetime.strptime(date_str_val, '%Y-%m')
    except ValueError: pass
    ...
    try: return datetime.strptime(date_str_val, '%Y')
    except ValueError: pass
    logging.warning(f"Konnte kein valides Datum aus '{date_str_val}' für die Sortierung parsen.")
    return datetime.min

def extract_year(date_str):
    """Extrahiert nur die vierstellige Jahreszahl aus einem Datumsstring."""
...
def format_authors(creators, single_author_mode=False):
    ...
    if not isinstance(creators, list):
        logging.warning(f"Unerwartetes Format für 'creators': {creators}.")
        return ""
    for creator in creators:
        if isinstance(creator, dict) and creator.get('creatorType') == 'author':
            ...
            name_str = ""
            if last_name and first_name: name_str = f"{last_name}, {first_name}"
            elif last_name: name_str = last_name
            elif first_name: name_str = first_name
            elif creator.get('name'): name_str = str(creator['name']).strip()
            if name_str: author_list.append(name_str)
    if not author_list: return ""
    if single_author_mode:
        return f"{author_list[0]} et al." if len(author_list) > 1 else author_list[0]
    else:
...
def find_best_link_json(item_data, fallback_url=None):
    ...
    if not isinstance(item_data, dict):
        logging.warning(f"Unerwartetes Format für item_data: {item_data}")
        return fallback_url
        ...
        if doi_text:
            ...
            if doi_text.startswith('http://doi.org/') or doi_text.startswith('https://doi.org/'):
                try:
                    ...
                    logging.error(f"Fehler beim Parsen der DOI-URL '{doi_text}': {e}")
    ...
    if url and str(url).strip().startswith(('http://', 'https://')):
        try:
            ...
            logging.warning(f"Konnte URL '{url}' nicht sicher parsen, verwende unverändert: {e}")
            return str(url).strip()
    item_key = item_data.get('key', 'unbekannt')
    if fallback_url:
        logging.info(f"Kein spezifischer Link für Item mit Key {item_key} gefunden. Verwende Fallback-URL.")
        return fallback_url
    else:
        logging.warning(f"Kein spezifischer Link und kein Fallback-URL für Item mit Key {item_key} gefunden.")
        return None

def get_categories_json(item_data, date_str):
    """Extrahiert Kategorien für einen RSS-Eintrag."""
    categories = []
    year = extract_year(date_str)
    if year: categories.append(year)
    if date_str:
        date_str_val = str(date_str).strip()
...
def process_item(item, single_author_mode, fallback_url):
    """Verarbeitet ein einzelnes Zotero-Item und gibt ein aufbereitetes Dictionary zurück."""
    ...
    processed_item = {
        ...
        'link': find_best_link_json(item_data, fallback_url=fallback_url),
        'categories': get_categories_json(item_data, date_str),
    }
    processed_item['identifier'] = get_unique_item_identifier(processed_item)
    return processed_item


def fetch_zotero_items(group_id_param, item_type_param, sort_by_param, direction_param, limit_param,
                       collection_key_override=None):
    """Holt alle Einträge von Zotero für eine spezifische Gruppe oder Collection.
    Diese Funktion verarbeitet oder sortiert die Daten NICHT weiter.
    """
    all_raw_items = []
    start = 0
    total_results = None
    ...
    while True:
        ...
        try:
            logging.info(f"Rufe API ab: Start={start}, URL={fetch_url}")
            headers = {'Zotero-API-Version': '3'}
            response = requests.get(fetch_url, params=params, headers=headers, timeout=120)

            if response.status_code != 200:
                logging.error(f"Fehler bei API-Abruf. Status: {response.status_code}, URL: {response.url}")
                return None
            ...
            if not items_json: break
            
            all_raw_items.extend(items_json)
            
            start += len(items_json)
            if total_results is not None and start >= total_results: break
            if len(items_json) < limit_param: break
        except Exception as e:
            logging.error(f"Fehler während API-Abruf: {e}", exc_info=True)
            break
            
    return all_raw_items

def fetch_and_deduplicate_all_ag_items(group_id_param, ag_configurations_param, single_author_mode):
    """
    Sammelt Items aus ALLEN AG-Collections, dedupliziert sie und sortiert sie.
    Wird für den Haupt-Feed des Instituts verwendet.
    """
    logging.info("Sammle und dedupliziere Einträge aus allen AGs für den Haupt-Feed...")
    all_items = {} # Dictionary zur Deduplizierung, Key = unique identifier, Value = item_data
    
    for ag_config in ag_configurations_param:
        ag_key = ag_config["key"]
        ag_label = ag_config["label"]
        logging.info(f"--> Sammle Einträge von: {ag_label}")
        
        raw_items = fetch_zotero_items(
            group_id_param=group_id_param,
            item_type_param=ZOTERO_ITEM_TYPE,
            sort_by_param=SORT_BY,
            direction_param=DIRECTION,
            limit_param=MAX_LIMIT_PER_REQUEST,
            collection_key_override=ag_key
        )
        
        if raw_items:
            fallback_url = f"https://www.zotero.org/groups/{group_id_param}/collections/{ag_key}/items"
            for item in raw_items:
                processed_item = process_item(item, single_author_mode, fallback_url)
                if processed_item and processed_item['identifier']:
                    # Wenn der Identifier noch nicht im Dictionary ist, füge das Item hinzu.
                    # Dies sorgt für die Deduplizierung.
                    if processed_item['identifier'] not in all_items:
                        all_items[processed_item['identifier']] = processed_item

    deduplicated_list = list(all_items.values())
    deduplicated_list.sort(key=lambda item: item['parsed_date'], reverse=True)
    
    logging.info(f"Nach Deduplizierung: {len(deduplicated_list)} einzigartige Einträge für Haupt-Feed gefunden.")
...
def create_rss_feed(items_data, output_filename_param, channel_title_param, channel_link_param,
    ...
    if channel_language_param: ET.SubElement(channel, 'language').text = channel_language_param
    ...
    for item_data in items_data:
        ...
        title_parts = []
        authors, year, paper_title, journal_name, volume_number = item_data.get('authors'), item_data.get('year'), item_data.get('title', '[Titel nicht verfügbar]'), item_data.get('journal'), item_data.get('volume')
        ...
        for i, part in enumerate(title_parts):
            ...
            if i == 0: rss_item_title += part_str
            elif part_str.startswith('('): rss_item_title += f" {part_str}"
            else:
                if rss_item_title and not rss_item_title.strip().endswith(')'): rss_item_title += "."
                rss_item_title += f" {part_str}"
        ET.SubElement(item, 'title').text = rss_item_title.strip()
        
        rss_link = item_data.get('link')
        if rss_link: ET.SubElement(item, 'link').text = rss_link
        else: logging.warning(f"Item '{rss_item_title[:50]}...' ({generator_label_param}) hat keinen Link.")
        ...
        if not guid_text and rss_link:
            ...
            guid_is_permalink = "false"
            logging.warning(f"Item '{rss_item_title[:50]}...' ({generator_label_param}) hat keine GUID.")
    ...
    try:
        ...
        logging.error(f"Fehler beim Schreiben der RSS-Datei '{output_filename_param}': {e}")
        return False
...
def generate_feeds_for_mode(single_author_mode):
    ...
    # 1. Haupt-Feed generieren (mit Deduplizierung)
    ...
    if not single_author_mode:
        main_output_filename = MAIN_FEED_FILENAME
    ...
    # NEUE Logik: Rufe die deduplizierende Funktion für den Haupt-Feed auf.
    deduplicated_main_data = fetch_and_deduplicate_all_ag_items(GROUP_ID, AG_CONFIGURATIONS, single_author_mode)
    
    if deduplicated_main_data:
        create_rss_feed(
            items_data=deduplicated_main_data, output_filename_param=main_output_filename,
    ...
    # 2. Feeds für Arbeitsgruppen generieren (ohne Deduplizierung)
    ...
    for ag_config in AG_CONFIGURATIONS:
        ...
        # Holt nur die rohen Items für die spezifische AG
        raw_ag_items = fetch_zotero_items(
            group_id_param=GROUP_ID, item_type_param=ZOTERO_ITEM_TYPE, sort_by_param=SORT_BY,
            direction_param=DIRECTION, limit_param=MAX_LIMIT_PER_REQUEST,
            collection_key_override=ag_key
        )
        
        if raw_ag_items:
            # Verarbeitet die rohen Items, sortiert sie und erstellt den Feed
            processed_ag_items = []
            fallback_url = f"https://www.zotero.org/groups/{GROUP_ID}/collections/{ag_key}/items"
            for item in raw_ag_items:
                processed_item = process_item(item, single_author_mode, fallback_url)
                if processed_item:
                    processed_ag_items.append(processed_item)
            
            processed_ag_items.sort(key=lambda item: item['parsed_date'], reverse=True)

            create_rss_feed(
                items_data=processed_ag_items, output_filename_param=current_output_filename,
